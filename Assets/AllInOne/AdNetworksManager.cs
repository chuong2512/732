...
namespace Oblius.Assets.AllInOneAdnetworks
{
	public class AdNetworksManager : MonoBehaviour
	{
...
#endif
        public int showInterstitialEveryXCalls=3;
        public static int interstitialCalls;
        ...
        {
            interstitialCalls++;
            if (interstitialCalls < showInterstitialEveryXCalls)
            {
                Debug.Log(interstitialCalls + "/" + showInterstitialEveryXCalls + " calls for interstitial to show");
                return;
            }
            Debug.Log(interstitialCalls + "/" + showInterstitialEveryXCalls + " calls for interstitial to show: COMPLETED. Calling Interstitial");

            interstitialCalls = 0;

#if UNITY_EDITOR
            Debug.Log("Done. Not calling the interstitial during Editor");
            onAdClosed();
			return;
#endif
